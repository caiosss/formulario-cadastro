...
export default function Imovel() {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<ImovelForm>({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 gap-4 p-4 rounded-lg bg-white shadow-xl m-3">
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <div className="col-span-6 flex flex-col gap-2">
          ...
          <Input />
          {errors.quadra && <span className="text-red-500 text-xs">{errors.quadra.message}</span>}
        </div>

        <div className="col-span-6 flex flex-col gap-2">
          <label className="font-medium">Lote(s):</label>
          <Input />
          {errors.lotes && <span className="text-red-500 text-xs">{errors.lotes.message}</span>}
        </div>
      </div>
    </form>
  );
